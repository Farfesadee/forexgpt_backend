...
class SignalService:
    ...
    def __init__(self, runpod_endpoint_id: str, runpod_api_key: str):
        """
        Args:
            runpod_endpoint_id: Runpod Endpoint ID for forexgpt finetuned model for inferenc
            runpod_api_key:       Mistral model ID (swap for HuggingFace model when ready)
        """
        # self.hf_client = hf_client
        self.runpod_endpoint_id = runpod_endpoint_id
        self.runpod_api_key = runpod_api_key
        self.runpod_url = f"https://api.runpod.ai/v2/{runpod_endpoint_id}/runsync"
        ...
        self.system_prompt = (
            ...
            'ONLY a valid JSON object. No markdown, no bullet points, no explanation text. '
            'Just the raw JSON starting with { and ending with }.\n\n'

            '## Signal Validity Rules — ALL must be met to set signal=true:\n'
            '1. A SPECIFIC currency or currency pair must be explicitly named (e.g. euro, yen, EUR/USD). '
            'Generic mentions of "foreign exchange headwinds" are NOT sufficient.\n'
            '2. A QUANTIFIED impact must be present (e.g. X percent revenue impact, Xm FX headwind). '
            'Vague language like "some exposure" does NOT qualify.\n'
            '3. A DIRECTIONAL implication must be clear — the transcript must indicate strengthening '
            'or weakening of a specific currency.\n'
            '4. The impact must be MATERIAL — at least moderate in scale relative to company revenue.\n\n'

            '## Hedge Interpretation Rules\n'
            '- If the transcript mentions a hedging program (forward contracts, options, derivatives) '
            'covering a specific currency or regional revenue, infer the effective currency exposure after hedging.\n'
            '- Use hedge language to determine direction: "EUR/USD position largely neutralized" implies '
            'protection against EUR weakening vs USD.\n'
            '- If hedge language reduces risk substantially, confidence can be increased up to 0.85 '
            'if all other rules are strongly met.\n'
            '- Only apply hedge inference if the underlying currency or region is explicitly mentioned.\n\n'
            ...
            '"reasoning": "exact quote from transcript supporting the signal, or which rule failed if signal=false", '
            '"magnitude": "low" or "moderate" or "high" or null, '
            '"time_horizon": "current_quarter" or "next_quarter" or "long_term" or null}\n\n'

            '## Confidence Scoring\n'
            'Start at 0.5 for a valid signal. Deduct 0.1 for each missing detail '
            '(no exact percentage, no time frame, indirect language). '
            'Increase up to 0.85 if hedge language confirms net exposure. '
            'Set confidence to null if signal=false.\n\n'

            'Be conservative. When in doubt, set signal=false.'
        )
    ...
    async def extract_signal(
        ...
        try:
            ...
            messages = [
                {"role": "system", "content": self.system_prompt},
                {"role": "user",   "content": self._build_extraction_prompt(
            ...
            if save_to_db and signal_data["signal"] and signal_data.get("currency_pair"):
                ...
                saved = db.signals.create(user_id, {
                    # signal_data.get("currency_pair"),
                    "currency_pair":      [signal_data.get("currency_pair")] if signal_data.get("currency_pair") else None,
            ...
            logger.info(
                f"Signal extraction complete — signal={signal_data['signal']}")
            return signal_data
    ...
    def get_user_signals(
        ...
        try:
            rows = db.signals.list(
                user_id=user_id, pair=currency_pair, direction=direction, limit=limit)
            for row in rows:
                row["currency_pair"] = self._normalize_currency_pair(
    ...
    def get_signal_statistics(self, user_id: str) -> Dict:
        ...
        try:
            ...
            for s in signals:
                pair = self._normalize_currency_pair(
                    s.get("currency_pair")) or "UNKNOWN"
                stats["by_currency_pair"][pair] = stats["by_currency_pair"].get(
                    pair, 0) + 1
    ...
    def _build_extraction_prompt(self, transcript: str, company_name: Optional[str]) -> str:
        ...
        return (
            f"Analyze this{company} earnings call transcript for forex signals. "
    ...
    def _extract_json(self, text: str) -> str:
        ...
        # If we get here, braces don't match - JSON is incomplete
        logger.warning(
            f"Unmatched braces in response: {text[start:start+100]}...")
        return text[start:]
    ...
    def _is_truncated(self, text: str) -> bool:
        ...
        """
        text = text.strip()
        # Check for unclosed strings, unclosed braces, or ending with comma/colon
        ...
        try:
            ...
            if not json_str:
                ...
                return {
                    ...
                    "direction":     None,
                    "confidence":    None,
                    "reasoning":     "No valid JSON found in model output",
                    "magnitude":     None,
                    "time_horizon":  None,
                }
            ...
            data["currency_pair"] = self._normalize_currency_pair(
                data["currency_pair"])
            ...
            logger.error(
                f"JSON parse error. Raw response: {response[:500]}...")
            return {
                "signal":        False,
                "currency_pair": None,
                "direction":     None,
    ...
    async def _generate_signal(self, messages: List[Dict]) -> str:
        ...
        for poll_count in range(max_polls):
            try:
                async with httpx.AsyncClient(verify=ssl_context, timeout=10.0) as client:
                    ...
                    logger.info(
                        f"Poll {poll_count + 1}: Job {job_id} status = {status}")
                    ...
                    if status == "COMPLETED":
                        ...
                        if output and len(output) > 0:
                            ...
                            if choices:
                                # Try text field first, then tokens
                        ...
                        raise RuntimeError(
                            f"Invalid output format: {status_data}")
        ...
        raise TimeoutError(
            f"Job {job_id} did not complete within {max_polls * poll_interval} seconds")
    ...
    #             last_error = e
    #             logger.warning(
    #                 f"_generate_signal attempt {attempt + 1} failed: {e}")
    #             if attempt < 2:
    #                 await asyncio.sleep(2)
